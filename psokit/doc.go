...
to give background information  followed by the godoc documentation.
 ...
 man which manages the sequence of runs to get some impression of how the chosen
 SPSO performs with the chosen cost function with Actions that are available for
 monitoring the runs. All these components (SPSO,Function,Actions) are chosen by
 name using various Select methods. There are short descriptions against each
 name and the toolkit provides methods for listing the names and their
